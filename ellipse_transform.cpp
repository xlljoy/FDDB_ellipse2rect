...
void ellipse_transform::ellipse_trans(int index)
{
    ...
    {
        Mat img_correct = imread(root_img_path_ + img_path_[index] + ".jpg");
        cur_img_cols_ = img_correct.cols;
        cur_img_rows_ = img_correct.rows;
...
//
        ...
        //rect.y = (0 > ellip[4] - rect.height / 2) ? 0 : ellip[4] - rect.height / 2;
        if(rect.x + rect.width > cur_img_cols_) rect.width = cur_img_cols_ - rect.x;
        if(rect.y + rect.height > cur_img_cols_) rect.height = cur_img_rows_ - rect.y;
...
void ellipse_transform::img_crop_create(int index) {
    ...
    for (int i = 0; i < cur_rect_.size(); i++) {
        for (int j = 0; j < cascade_rects_[index].size(); j++) {
            float weight = IoM(cur_rect_[i], cascade_rects_[index][j]);
            if (IoM(cur_rect_[i], cascade_rects_[index][j]) >= 0.4) //threshold
            {
...
void ellipse_transform::txt_write(string type, string img_path, vector<Rect> rectangles) {
    ...
    {
        file << root_img_path_ << img_path <<endl
...
void ellipse_transform::txt_write(string type, string img_path) {

    int label = 1;
    if(type == "negative") label = 0;
    fstream file(root_path_ + type + "/file_list.txt", ios::app);
    string label_s = to_string(label);
    file << root_img_path_ << img_path << " "<<label_s<< endl;
}
...
//                cv::FONT_HERSHEY_COMPLEX_SMALL, 0.8, cvScalar(255, 0, 0), 1, CV_AA);
        if(rectangles[i].x + rectangles[i].width > cur_img_cols_) rectangles[i].width = cur_img_cols_ - rectangles[i].x;
        if(rectangles[i].y + rectangles[i].height > cur_img_rows_) rectangles[i].height = cur_img_rows_ - rectangles[i].y;
        img_crop.push_back(img(rectangles[i]));
    }
    if(img_crop.size() != 0) {
        string img_aim_path = root_path_ + type + "/" + img_path_[index];
        create_folder(img_aim_path);
        for (int j = 0; j < img_crop.size(); j++) {
            string num = to_string(j);
            imwrite(img_aim_path + "/" + "CascadeCNN" + num + ".jpg", img_crop[j]);
            txt_write(type,img_path_[index]);
        }
    }
}
...
void ellipse_transform::create_folder(string dir_path)
{
    //boost::filesystem::path dir(dir_path);
    if (boost::filesystem::create_directories(dir_path)) {
        cout << "Folder : " << dir_path << endl;
    }
}
